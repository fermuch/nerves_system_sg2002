...
if [ -f "$HOST_DIR/bin/mkenvimage" ] && [ -f "$NERVES_DEFCONFIG_DIR/uboot/uboot-emmc.env" ]; then
    echo "Generating uboot-env.bin..."
    $HOST_DIR/bin/mkenvimage -s 131072 -o $BINARIES_DIR/uboot-env.bin $NERVES_DEFCONFIG_DIR/uboot/uboot-emmc.env
